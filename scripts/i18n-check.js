...
function extractTranslationKeys(fileContent) {
  ...
  // 先移除注释，避免识别注释中的内容
  const codeContent = removeComments(fileContent);
  ...
  while ((match = withTemplateDefaultPattern.exec(codeContent)) !== null) {
    const key = match[1];
    if (isValidKey(key)) {
      keys.push({
        key,
        defaultValue: match[2],
        line: codeContent.substring(0, match.index).split('\n').length,
      });
    }
  }

  // 再匹配带 defaultValue 的（普通字符串）
  while ((match = withDefaultPattern.exec(codeContent)) !== null) {
    const key = match[1];
    if (isValidKey(key)) {
      keys.push({
        key,
  ...
  // 最后匹配简单的 t("key")
  while ((match = simplePattern.exec(codeContent)) !== null) {
    const key = match[1];
    if (!isValidKey(key)) continue;

    // 检查是否已经被前面的模式匹配过
    const alreadyExists = keys.some(k => k.key === key && Math.abs(k.line - codeContent.substring(0, match.index).split('\n').length) <= 1);
    if (!alreadyExists) {
      keys.push({
        key,
        defaultValue: null,
...
// 获取 key 在语言对象中的值
function getKeyValue(localeData, key) {
...
// 检查 key 是否存在于语言对象中（且值不为空字符串）
function hasKey(localeData, key) {
  const value = getKeyValue(localeData, key);
...
function translateDefaultValue(defaultValue, targetLang) {
  ...
  // 对于非中文语言，使用中文默认值并添加标记提示需要翻译
  // 开发者应该手动将这些标记的文本翻译成对应语言
  return `[TODO:${targetLang.toUpperCase()}] ${defaultValue}`;
}
