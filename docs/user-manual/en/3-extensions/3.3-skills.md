...
CC AITools comes pre-configured with the following GitHub repositories:
...
CC AITools provides powerful search and filter features:
...
- **Automatic backup**: Before deletion, the skill is backed up to `~/.cc-aitools/skill-backups/`
- Removes the skill from all app directories (Claude, Codex, Gemini, OpenCode)
- Removes the skill from the SSOT directory (`~/.cc-aitools/skills/`)
