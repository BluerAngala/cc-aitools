...
App takeover means letting CC AITools's proxy intercept and forward a specific application's API requests.
...
When takeover is enabled, CC AITools modifies the app's configuration file to point the API endpoint to the local proxy.
...
When disabling takeover, CC AITools will:
...
Before enabling takeover, CC AITools backs up the original configuration and restores it when disabled.
