...
The CLI tools managed by CC AITools (Claude Code, Codex, Gemini CLI) require a Node.js environment.
...
1. Visit the [Releases page](https://github.com/BluerAngala/cc-aitools/releases)
...
2. Extract to get `CC AITools.app`
3. Drag it to the Applications folder

### Signed and Notarized

CC AITools for macOS is signed and notarized by Apple. You can install and open it directly — no extra steps needed.
...
After installation, launch CC AITools:

1. The app window displays correctly
2. A CC AITools icon appears in the system tray
3. You can switch between Claude / Codex / Gemini apps

## Auto Update

CC AITools includes built-in auto-update functionality:
...
- Move `CC AITools.app` to Trash
- Optional: Delete the configuration directory `~/.cc-aitools/`

### Linux

```bash
# Debian/Ubuntu
sudo apt remove cc-aitools

# ArchLinux
paru -R cc-aitools-bin
```
