...
This section describes how to configure CC AITools according to your preferences.
...
CC AITools supports three languages:
...
When enabled, CC AITools automatically runs when the system starts.
...
When enabled, CC AITools automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that CC AITools uses when opening a terminal.
...
The storage location for CC AITools's own data, defaulting to `~/.cc-aitools/`.
...
Configure CC AITools's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, CC AITools automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `cc-aitools-sync`) |
...
- CC AITools automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, CC AITools displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current CC AITools version number, with support for:
