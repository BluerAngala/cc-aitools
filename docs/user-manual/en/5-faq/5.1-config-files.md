...
## CC AITools Data Storage

### Storage Directory

Default location: `~/.cc-aitools/`
...
```
~/.cc-aitools/
├── cc-aitools.db      # SQLite database (SSOT)
...
`cc-aitools.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to CC AITools's "providers") |
...
CC AITools's priority when modifying configurations:

1. **CC AITools Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by CC AITools)
- CC AITools's `settings.json`

### Not Recommended to Edit Manually

- `cc-aitools.db` database file
- Backup files
...
CC AITools v3.7.0 migrated from JSON files to SQLite:
