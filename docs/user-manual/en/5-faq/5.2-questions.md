...
CC AITools for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/BluerAngala/cc-aitools/releases).
...
2. Add CC AITools to your antivirus software's whitelist
...
**Solutions**:
1. Check if the `~/.cc-aitools/` directory exists
2. Restore from backup: `~/.cc-aitools/backups/`
...
**Solutions**:
1. Confirm the file was exported by CC AITools
...
**Windows**:
- Check taskbar settings to ensure the CC AITools icon is not hidden
...
2. Restart the app
3. Delete `~/.cc-aitools/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask cc-aitools`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and CC AITools runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
1. Visit [GitHub Issues](https://github.com/BluerAngala/cc-aitools/issues)
...
- macOS/Linux: `~/.cc-aitools/logs/`
- Windows: `%APPDATA%\cc-aitools\logs\`
