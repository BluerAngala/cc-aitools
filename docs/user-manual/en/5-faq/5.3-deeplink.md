...
CC AITools supports the `ccswitch://` deep link protocol, enabling one-click configuration import via links.
...
CC AITools provides an online deep link generator tool:

**URL**: [https://BluerAngala.github.io/cc-aitools/deplink.html](https://BluerAngala.github.io/cc-aitools/deplink.html)
...
Using CC AITools's official online deep link generator tool is more convenient.
...
1. The system asks whether to open CC AITools
2. After confirming, CC AITools opens
...
CC AITools automatically registers the `ccswitch://` protocol during installation.
...
```bash
/usr/bin/open -a "CC AITools" --args --register-protocol
```
