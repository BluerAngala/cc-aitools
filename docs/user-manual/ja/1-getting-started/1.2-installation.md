...
CC AITools が管理する CLI ツール（Claude Code、Codex、Gemini CLI）には Node.js 環境が必要です。
...
1. [Releases ページ](https://github.com/BluerAngala/cc-aitools/releases) にアクセス
...
2. 展開して `CC AITools.app` を取得
3. 「アプリケーション」フォルダにドラッグ

### 署名・公証済み

CC AITools の macOS 版は Apple のコード署名と公証を受けています。追加の操作なしで直接インストールして開くことができます。
...
インストール完了後、CC AITools を起動します：

1. アプリウィンドウが正常に表示される
2. システムトレイに CC AITools のアイコンが表示される
3. Claude / Codex / Gemini の 3 つのアプリを切り替えられる

## 自動更新

CC AITools には自動更新機能が内蔵されています：
...
- `CC AITools.app` をゴミ箱に移動
- オプション：設定ディレクトリ `~/.cc-aitools/` を削除
