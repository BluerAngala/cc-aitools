...
CC AITools は `ccswitch://` ディープリンクプロトコルをサポートしており、リンクからワンクリックで設定をインポートできます。
...
CC AITools はオンラインのディープリンク生成ツールを提供しています：

**アクセス先**：[https://BluerAngala.github.io/cc-aitools/deplink.html](https://BluerAngala.github.io/cc-aitools/deplink.html)
...
CC AITools 公式のオンラインディープリンク生成ツールを使用するとより便利です。
...
1. システムが CC AITools を開くかどうかを確認
2. 確認後、CC AITools が起動
3. インポート確認ダイアログを表示
4. インポートを確認
...
CC AITools のインストール時に `ccswitch://` プロトコルが自動登録されます。
