...
```
~/.cc-aitools/
├── cc-aitools.db      # SQLite データベース（SSOT）
...
`cc-aitools.db` は SQLite データベースで、以下を保存しています：
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（CC AITools の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
CC AITools が設定を変更する際の優先順位：

1. **CC AITools データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（CC AITools がバックフィルする）
- CC AITools の `settings.json`

### 手動編集を推奨しないもの

- `cc-aitools.db` データベースファイル
- バックアップファイル
...
CC AITools v3.7.0 で JSON ファイルから SQLite に移行しました：
