...
Added full reverse proxy support for GitHub Copilot, enabling Copilot-authenticated requests to be forwarded through CC AITools.
...
- Backups stored in `~/.cc-aitools/skill-backups/` with all skill files and a `meta.json` containing original metadata
...
Users enable this feature **at their own risk**. CC AITools is not responsible for any account restrictions, warnings, or service suspensions resulting from the use of this feature.

---

## Download & Installation

Visit [Releases](https://github.com/BluerAngala/cc-aitools/releases/latest) to download the appropriate version.
